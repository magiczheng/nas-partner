...
export const ddns = {
  ...
  toggle: (id: number) => api.post<DDNSConfig>(`/ddns/${id}/toggle`, undefined),
  run: (id: number) => api.post<DDNSRunLog>(`/ddns/${id}/run`, undefined),
  ...
  listLogs: (configId: number, limit?: number) => api.get<DDNSRunLog[]>(`/ddns/${configId}/logs?limit=${limit || 20}`),
  clearLogs: (configId: number) => api.delete<void>(`/ddns/${configId}/logs`),
  cleanupLogs: () => api.post<{ deleted: number }>('/ddns/logs/cleanup', undefined),
};
